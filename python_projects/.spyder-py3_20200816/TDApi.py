...
class TDClient():
    ...
    # Remove stop flag or any file if older than max_ttl allowed
    # This is originally designed to resume the renewal of authentication key
    def del_stop_flag(self, **kwargs):
        for self.key in kwargs:
            if self.key == "full_path_file":
                self.my_full_path_file = kwargs[self.key]         
            elif self.key == "max_ttl":
                self.max_ttl_allowed = kwargs[self.key]     
        timeObj = timeConverter()
        if os.path.isfile(self.my_full_path_file) and float(timeObj.file_age(self.my_full_path_file)) > float(self.max_ttl_allowed):
            os.remove(self.my_full_path_file)
    ...
    def get_cache(self) -> dict:
        # Remove stop flag if it is more than 1m old.
        # This 1m old is to assume that's how long the useAuthCode_getAccessKey
        # take to get the new key and well beyond the time useRefreshToken_renewAccessKey needs
        # to request new access key
        # When stop file exists more than 1m, it's an indication that useRefreshToken_renewAccessKey
        # was interrupted and never finished
        self.del_stop_flag(full_path_file = self.config['tda_stop_flag_file'], max_ttl = 60)
    ...
    def dbConnector(self):
        ...
        self.cnxn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server}; \
                       ...
                       SQL_COPT_SS_MARS_ENABLED, SQL_MARS_ENABLED_YES')
    ...
    def dbTableCreate(self, ticker):
        ...
        # Import text object to double quote ticker with restricted characters
        ...
        self.tbcheck_query = ''' IF NOT (EXISTS (SELECT * FROM sysobjects WHERE name='{db_table}' and xtype='U'))  
                                 ...
                                 END;'''.format(db_table = self.textObj.convertSQLticker(self.myticker))
    ...
    def dbInsertTicker(self, quotestring):
        ...
        self.insert_quote = '''INSERT INTO {db_table}(
                            ...
                            delayed) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,? );'''.format(db_table = self.textObj.convertSQLticker(self.quotestring['symbol']))
        ...
        del self.db_cursor, self.db_connector
    ...
    def dbInsertMultiple(self, big_quote):
        ...
        for self.ticker_quote_key in self.mybig_quote:
            ...
            self.dbInsertTicker(self.ticketQuote)
