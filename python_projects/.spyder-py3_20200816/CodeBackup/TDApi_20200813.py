...
class TDClient():
    ...
    def useAuthCode_getAccessKey(self):
        ...
        if self.tda_authReply.status_code == 200:
            ...
            self.refresh_token_time_received = time.time();
    ...
    def accessToken_expires_at(self):
        self.my_TDA_cache = self.get_cache()
        self.access_token_expire = time.time() + int(self.my_TDA_cache['expires_in'])
    ...
    def refreshToken_expires_at(self):
        self.my_TDA_cache = self.get_cache()
        self.refresh_token_expire = self.refresh_token_time_received + int(self.my_TDA_cache['refresh_token_expires_in'])
    ...
    def accessToken_ttl(self):
        ...
        if 60 >= int(self.my_TDA_cache['expires_in']):
            return 0
        else:
            # return time to live of access token otherwise
            return int(int(self.my_TDA_cache['expires_in']) - 60)
    def refreshToken_ttl(self):
        ...
        if time.time() + 60 >= self.refresh_token_time_received + int(self.my_TDA_cache['refresh_token_expires_in']):
            return 0
        else:
            # return time to live of refresh token otherwise
            return int(int(self.my_TDA_cache['refresh_token_expires_in']) - 60)
    ...
    def useRefreshToken_renewAccessKey(self):
        ...
        if self.tda_authReply.status_code == 200:
            ...
            # convert jason string to dictionary
            self.tda_decoded_auth_content = self.tda_authReply.json()
            print("Type of self.tda_decoded_auth_content :", type(self.tda_decoded_auth_content))
            # Append refresh token and 'refresh token expires in' (ttl) to the 
            # returned URL
            #tda_decoded_content
            self.tda_decoded_auth_content.update({'refresh_token': self.my_tda_refresh_token,'refresh_token_expires_in':self.my_tda_refresh_token_ttl})
            ...
            with open(self.config['tda_accessToken_cache_file'], 'w') as self.writeobj:
                json.dump(self.tda_decoded_auth_content, self.writeobj)
                self.writeobj.close()
            print("This line shows the block was successful")
        else:
            print("Status code: ", self.tda_authReply.status_code)
            print("MAJOR ERROR: Could not get access code!!! Check your browser login")
            raise KeyError ('MAJOR ERROR!!!! COULD NOT GET ACCESS CODE VIA BROWSER LOGIN \
